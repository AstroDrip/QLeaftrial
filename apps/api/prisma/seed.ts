import argon2 from "argon2";
import { pathToFileURL } from "node:url";
import { prisma } from "../src/lib/prisma.ts";

const products = [
  {
    slug: "house-plant",
    sku: "QL-HP-001",
    name: "House Plant",
    nameAr: "نبات منزلي",
    description: "A generous, glossy-leafed plant for bright living spaces.",
    descriptionAr: "نبات بأوراق لامعة وكثيفة يناسب مساحات المعيشة ذات الإضاءة الساطعة.",
    category: "Indoor",
    categoryAr: "داخلي",
    light: "Bright indirect",
    lightAr: "إضاءة ساطعة غير مباشرة",
    priceQar: 180,
    costPrice: 90,
    quantity: 12,
    media: { url: "/media/plants/house-plant.jpg", altText: "House plant in a ceramic pot" },
  },
  {
    slug: "fiddle-leaf-fig",
    sku: "QL-FF-002",
    name: "Fiddle Leaf Fig",
    nameAr: "فيكس ليراتا",
    description: "Sculptural leaves that make a calm, vertical statement.",
    descriptionAr: "أوراق نحتية تمنح المساحة حضورًا رأسيًا هادئًا.",
    category: "Indoor",
    categoryAr: "داخلي",
    light: "Bright indirect",
    lightAr: "إضاءة ساطعة غير مباشرة",
    priceQar: 260,
    costPrice: 130,
    quantity: 8,
    media: { url: "/media/plants/fiddle-leaf-fig.jpg", altText: "Fiddle leaf fig by a window" },
  },
  {
    slug: "snake-plant",
    sku: "QL-SP-003",
    name: "Snake Plant",
    nameAr: "نبات الثعبان",
    description: "An upright, resilient plant for low-maintenance corners.",
    descriptionAr: "نبات قوي قائم يناسب الزوايا التي تحتاج إلى عناية بسيطة.",
    category: "Indoor",
    categoryAr: "داخلي",
    light: "Low to bright indirect",
    lightAr: "إضاءة منخفضة إلى ساطعة غير مباشرة",
    priceQar: 120,
    costPrice: 55,
    quantity: 16,
    media: { url: "/media/plants/snake-plant.jpg", altText: "Snake plant in a woven basket" },
  },
  {
    slug: "monstera-deliciosa",
    sku: "QL-MD-004",
    name: "Monstera Deliciosa",
    nameAr: "مونستيرا ديليسيوسا",
    description: "Split tropical leaves for a lush, layered interior.",
    descriptionAr: "أوراق استوائية مشقوقة تمنح الداخل مظهرًا غنيًا ومتدرجًا.",
    category: "Indoor",
    categoryAr: "داخلي",
    light: "Medium indirect",
    lightAr: "إضاءة متوسطة غير مباشرة",
    priceQar: 220,
    costPrice: 105,
    quantity: 10,
    media: { url: "/media/plants/monstera-deliciosa.jpg", altText: "Monstera deliciosa in a terracotta pot" },
  },
  {
    slug: "aloe-vera",
    sku: "QL-AV-005",
    name: "Aloe Vera",
    nameAr: "ألوفيرا",
    description: "A useful, sun-loving succulent with architectural leaves.",
    descriptionAr: "نبات عصاري محب للشمس بأوراق معمارية مميزة.",
    category: "Succulent",
    categoryAr: "عصاري",
    light: "Direct sun",
    lightAr: "شمس مباشرة",
    priceQar: 75,
    costPrice: 30,
    quantity: 20,
    media: { url: "/media/plants/aloe-vera.jpg", altText: "Aloe vera plant on a sunny shelf" },
  },
  {
    slug: "peace-lily",
    sku: "QL-PL-006",
    name: "Peace Lily",
    nameAr: "زنبق السلام",
    description: "Soft white blooms and deep foliage for quieter rooms.",
    descriptionAr: "أزهار بيضاء ناعمة وأوراق داكنة للمساحات الهادئة.",
    category: "Indoor",
    categoryAr: "داخلي",
    light: "Low to medium indirect",
    lightAr: "إضاءة منخفضة إلى متوسطة غير مباشرة",
    priceQar: 145,
    costPrice: 65,
    quantity: 14,
    media: { url: "/media/plants/peace-lily.jpg", altText: "Peace lily with white blooms" },
  },
] as const;

export async function seedDatabase(): Promise<void> {
  const password = adminPasswordForSeed();

  for (const product of products) {
    const { media, quantity, ...productData } = product;

    await prisma.product.upsert({
      where: { slug: product.slug },
      create: {
        ...productData,
        published: true,
        inventory: { create: { quantity } },
        media: { create: { ...media, sortOrder: 0 } },
      },
      update: {
        ...productData,
        published: true,
        inventory: { upsert: { create: { quantity }, update: { quantity } } },
        media: {
          deleteMany: {},
          create: { ...media, sortOrder: 0 },
        },
      },
    });
  }

  const developmentAdmin = await prisma.adminUser.findUnique({
    where: { email: "admin@qleaves.local" },
  });
  const passwordHash = developmentAdmin && await argon2.verify(developmentAdmin.passwordHash, password)
    ? developmentAdmin.passwordHash
    : await argon2.hash(password);

  await prisma.adminUser.upsert({
    where: { email: "admin@qleaves.local" },
    create: {
      email: "admin@qleaves.local",
      name: "QLeaves Development Admin",
      passwordHash,
    },
    update: {
      name: "QLeaves Development Admin",
      passwordHash,
    },
  });
}

function adminPasswordForSeed(): string {
  if (process.env.QLEAVES_DATABASE_PROVIDER !== "postgresql") {
    return "taimuomar";
  }

  const password = process.env.QLEAVES_ADMIN_SEED_PASSWORD?.trim();
  if (!password) {
    throw new Error(
      "QLEAVES_ADMIN_SEED_PASSWORD is required when seeding PostgreSQL",
    );
  }
  if (password === "taimuomar") {
    throw new Error(
      "QLEAVES_ADMIN_SEED_PASSWORD must not use the known local development password",
    );
  }
  if (password.length < 12) {
    throw new Error(
      "QLEAVES_ADMIN_SEED_PASSWORD must contain at least 12 characters",
    );
  }

  return password;
}

if (
  process.argv[1] &&
  import.meta.url === pathToFileURL(process.argv[1]).href
) {
  seedDatabase()
    .then(() => prisma.$disconnect())
    .catch(async (error: unknown) => {
      await prisma.$disconnect();
      console.error(error);
      process.exitCode = 1;
    });
}
